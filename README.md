...
在主用电脑上，本仓库直接就是 ZCode 工作区里的 `plugins/` 目录
（`D:\Zcode\.zcode\workspace\default\plugins`，仓库根与插件目录平级）。
这样本地开发、提交、多机同步用的是同一份文件，不需要来回拷贝。

注意两点：

- 工作区根目录（仓库的上一级）**不是**仓库，工作区里其他文件的改动不会被 git 跟踪。
- ZCode 插件创建工具默认会在 `plugins/` 下生成一份本地市场清单
  `plugins/marketplace.json`，它不在本仓库里（上游已改用根目录
  `.claude-plugin/marketplace.json`）；如果哪天它又出现了，保持其不被提交即可。

在其他电脑上同步插件时，直接克隆本仓库到任意位置即可。
...
1. 在仓库根下建目录，目录名与插件名一致（小写 + 连字符）。
2. 写 `<name>/.zcode-plugin/plugin.json`：`name`、`version`、`description`、`skills`，
...
4. 在 `.claude-plugin/marketplace.json` 的 `plugins` 里登记一条，`source` 写 `./<name>`，
